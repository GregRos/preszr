import { ResultType, ScalarValue, tryDecodeScalar } from "../data"
import {
...
export class DecodeContext {
        ...
        switch (result.type) {
            case ResultType.Reference:
                if (result.value >= this._entityStream.length) {
                    throw decode_encoding_refOutOfBounds(this.self, result.value)
                }
    ...
    /**
     * A fragile way to decode values during the CREATE phase. Not meant to be called from user
     * code.
     *
