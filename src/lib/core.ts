...
export class Preszr {
    ...
    private _checkHeaderWellformed(input: PreszrFormat) {
        ...
        if (!Array.isArray(input)) {
            ...
            throw decode_badMessage(ParseError.message__length_less_than_2, input)
        } else {
            ...
            if (!header) {
                ...
                throw decode_badMessage(ParseError.message__header_not_array, input)
            } else {
                versionInfo = header[0]
                if (versionInfo == null) {
                    throw decode_badMessage(ParseError.message__header_empty, versionInfo)
                    ...
                    throw decode_badHeader(ParseError.header__key_list_not_array, header)
                } else if (!header[2]) {
                    throw decode_badHeader(ParseError.header__no_encoding_map, header)
                } else if (typeof header[2] !== "object" || Array.isArray(header[2])) {
                    throw decode_badHeader(ParseError.header__encoding_map_not_object, header)
                } else if (!header[3]) {
                    throw decode_badHeader(ParseError.header__no_metadata, header)
                } else if (typeof header[3] !== "object" || Array.isArray(header[3])) {
                    throw decode_badHeader(ParseError.header__metadata_not_object, header)
                } else if (header[4] == null) {
                    throw decode_badHeader(ParseError.header__no_root_reference, header)
                } else if (typeof header[4] !== "number" || !Number.isInteger(header[4])) {
                    throw decode_badHeader(ParseError.header__root_reference_not_integer, header)
                }
            }
        }
    ...
    private _checkHeaderContents(input: PreszrFormat) {
        ...
        for (const key in encodingSpec) {
            ...
            if (!isReference(key)) {
                throw decode_badHeader(ParseError.header__encoding_map_key_not_reference, key)
            }
            if (typeof val !== "number" || !Number.isInteger(val)) {
                throw decode_badHeader(ParseError.header__encoding_map_value_not_index, val)
            }

            if (val < 0 || val - FixedIndexes.End >= encodingKeys.length) {
                throw decode_badHeader(ParseError.header__encoding_map_index_out_of_bounds, input)
            }
            if (+key >= input.length) {
                throw decode_badHeader(ParseError.header__encoding_map_key_out_of_bounds, key)
            }
        }
        for (const key in metadata) {
            ...
            if (!isReference(key)) {
                throw decode_badHeader(ParseError.header__metadata_key_not_reference, key)
            }
            if (+key >= input.length) {
                throw decode_badHeader(ParseError.header__metadata_key_out_of_bounds, key)
            }
        }

        if (root < 0 || root >= input.length) {
            throw decode_badHeader(ParseError.header__root_reference_out_of_bounds, root)
        }
    }
