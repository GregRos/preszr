...
test("invalid definition - has name", t => {
    t.throws(() => Preszr([{ encodes: Date.prototype, name: "should_be_empty", version: 100 }]))
})
...
{
    const testWithModifiedDates = testBuilder(Preszr([modifiedDateEncoding])).get()
...
{
    const testWith2Overrides = testBuilder(Preszr([modifiedDateEnc2, modifiedDateEncoding])).get()
