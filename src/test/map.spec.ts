...
}), new Map([[o, o]]), [[{1: getLibraryString("Map")}, {}], [["2", "2"]], {}]);

test("two items", macro, new Map([[1, 1], [2, 2]]), [
    [{1: getLibraryString("Map")}, {}],
    [[1, 1], [2, 2]]
]);

test("1 unsupported key", encodeDecodeMacro({
    encode: testEncodeMacro,
    decode(t, decoded, encoded) {
        const rDecoded = decode(encoded);
    ...
    [{1: getLibraryString("Map"), 2: unsupportedEncodingKey}, {2: "Function"}],
    [["2", 1]],
    0
]);

test("2 unsupported keys", encodeDecodeMacro({
    encode: testEncodeMacro,
    decode(t, decoded, encoded) {
        const rDecoded = decode(encoded);
    ...
    [{1: getLibraryString("Map"), 2: unsupportedEncodingKey, 3: unsupportedEncodingKey}, {2: "Function", 3: "Function"}],
    [["2", 1], ["3", 2]],
    0,
    0
]);
